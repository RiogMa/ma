...
class TrainLoop:
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            mask_lul = mask_dict['left upper limb']
            mask_lul = np.expand_dims(np.expand_dims(mask_lul, axis=0), axis=(2, 3)) # [263] -> [1,263,1,1]
            mask_lul = np.tile(mask_lul,(micro.shape[0],1,1,1)) # [1, 263, 1,1] -> [bs, 263, 1, 1]
            mask_lul = torch.tensor(mask_lul)
            mask_lul = mask_lul.to(self.device)
            
            mask_rul = mask_dict['right upper limb']
            mask_rul = np.expand_dims(np.expand_dims(mask_rul, axis=0), axis=(2, 3)) # [263] -> [1,263,1,1]
            mask_rul = np.tile(mask_rul,(micro.shape[0],1,1,1)) # [1, 263, 1,1] -> [bs, 263, 1, 1]
            mask_rul = torch.tensor(mask_rul)        
            mask_rul = mask_rul.to(self.device)
            
            mask_lll = mask_dict['left lower limb']
            mask_lll = np.expand_dims(np.expand_dims(mask_lll, axis=0), axis=(2, 3)) # [263] -> [1,263,1,1]
            mask_lll = np.tile(mask_lll,(micro.shape[0],1,1,1)) # [1, 263, 1,1] -> [bs, 263, 1, 1]
            mask_lll = torch.tensor(mask_lll)
            mask_lll = mask_lll.to(self.device)
            
            mask_rll = mask_dict['right lower limb']
            mask_rll = np.expand_dims(np.expand_dims(mask_rll, axis=0), axis=(2, 3)) # [263] -> [1,263,1,1]
            mask_rll = np.tile(mask_rll,(micro.shape[0],1,1,1)) # [1, 263, 1,1] -> [bs, 263, 1, 1]
            mask_rll = torch.tensor(mask_rll)
            mask_rll = mask_rll.to(self.device)
            
            mask_torso = mask_dict['torso']
            mask_torso = np.expand_dims(np.expand_dims(mask_torso, axis=0), axis=(2, 3)) # [263] -> [1,263,1,1]
            mask_torso = np.tile(mask_torso,(micro.shape[0],1,1,1)) # [1, 263, 1,1] -> [bs, 263, 1, 1]
            mask_torso = torch.tensor(mask_torso)
            mask_torso = mask_torso.to(self.device)
            # 5个身体部位
            body_lul_0 = micro * mask_lul
            body_rul_0 = micro * mask_rul
            body_lll_0 = micro * mask_lll
            body_rll_0 = micro * mask_rll
            body_torso_0 = micro * mask_torso  
