...
from utils.get_core import get_core
from utils.get_core_generation import get_core as get_core_generation
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, name, core_parts,mode, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        if model_kwargs is None:
            model_kwargs = {}
        # core_body, non_core_body = []
        B, C = x.shape[:2]
        # core_body, non_core_body = get_core(x, model_kwargs, name)
        assert t.shape == (B,)
        # if mode == 'generation':
        core_body, non_core_body = get_core_generation(x, core_parts, name)
        # elif mode == 'eval':
        #     core_body, non_core_body = get_core(x, model_kwargs, name)
        model_output = model(core_body, non_core_body, self._scale_timesteps(t), **model_kwargs)
    ...
    def training_losses(self, model, body_vip, body_not, body_all,  name, t, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(body_all)
            
        body_vip_t = self.q_sample(body_vip, t, noise=noise)
        body_not_t = self.q_sample(body_not, t, noise=noise)
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            model_output = model(body_vip_t, body_not_t, self._scale_timesteps(t), **model_kwargs) 
            # body_not_output = model(body_not_t, self._scale_timesteps(t), **model_kwargs)
            ...
            #         x_start=body_vip, x_t=body_vip_t, t=t
            #     )[0],
            #     ModelMeanType.START_X: body_vip,
            #     ModelMeanType.EPSILON: noise,
            # }[self.model_mean_type]
            target = {
                # ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                #     x_start=body_all, x_t=body_not_t, t=t
                # )[0],
                ModelMeanType.START_X: body_all,
                ModelMeanType.EPSILON: noise
            }[self.model_mean_type]
            
            assert model_output.shape == target.shape == body_vip.shape  # [bs, njoints, nfeats, nframes]
            # assert body_not_output.shape == target_not.shape == body_not.shape  # [bs, njoints, nfeats, nframes]
            ...
            # 核心部位损失
            # '''
            # terms["rot_mse_vip"] = self.masked_l2(target_vip, body_vip_output, mask) # mean_flat(rot_mse)
            
            # if self.lambda_rcxyz > 0.:
            #     target_xyz = get_xyz(target_vip)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes]
            #     model_output_xyz = get_xyz(body_vip_output)  # [bs, nvertices, 3, nframes]
            #     terms["rcxyz_mse_vip"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            # if self.lambda_vel > 0.:
            #     target_xyz = get_xyz(target_vip) 
            #     model_output_xyz = get_xyz(body_vip_output)
            #     target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
            #     model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
            #     terms["vel_mse_vip"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            
            # terms["loss_vip"] = terms["rot_mse_vip"]+\
            #                 (self.lambda_vel * terms.get('vel_mse_vip', 0.)) +\
            # #                 (self.lambda_rcxyz * terms.get('rcxyz_mse_vip', 0.))
            # '''
            # 非核心部位损失
            # '''
            ...
            if self.lambda_vel > 0.:
                ...
                target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                terms["vel_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            ...
            # # print("vel: ", self.lambda_vel)
            # terms["loss"] = self.lambda_body * terms["loss_vip"] + self.lambda_general * terms["loss_not"]
