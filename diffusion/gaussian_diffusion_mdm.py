...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start_new, x_start_old, t, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start_new)
        x_t_new = self.q_sample(x_start_new, t, noise=noise)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            all_body_losses
            '''
            all_body_new = x_t_new + x_start_old
            all_body_old = x_start_new + x_start_old
            ...
            target = {
                ...
                ModelMeanType.START_X: all_body_new,
                ModelMeanType.EPSILON: noise,
            }[self.model_mean_type]
            assert model_output.shape == target.shape == all_body_old.shape  # [bs, njoints, nfeats, nframes]

            terms["rot_mse_all"] = self.masked_l2(target, all_body_old, mask) # mean_flat(rot_mse)

            terms["loss"] = terms["loss"] + terms["rot_mse_all"]
    ...
    def training_losses_local(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        """
        Compute training losses for a single timestep.
    ...
    def training_losses_all(self, model, x_start, x_start_new, t, model_kwargs=None, noise=None, dataset=None):
        """
        Compute training losses for a single timestep.
        ...
        x_t = self.q_sample(x_start, t, noise=noise)
        '''
        x_t = self.q_sample(x_start_new, t, noise=noise)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
            '''
            model_output_new = model(x_start_new, self._scale_timesteps(t), **model_kwargs)
            ...
            if self.model_var_type in [
                ...
                B, C = x_t.shape[:2]
                assert model_output_new.shape == (B, C * 2, *x_t.shape[2:])
                model_output_new, model_var_values = th.split(model_output_new, C, dim=1)
                # Learn the variance using the variational bound, but don't let
                # it affect our mean prediction.
                frozen_out = th.cat([model_output_new.detach(), model_var_values], dim=1)
                terms["vb"] = self._vb_terms_bpd(
                    model=lambda *args, r=frozen_out: r,
                    x_start_new=x_start_new,
                    x_t=x_t,
            ...
            target = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start_new=x_start_new, x_t=x_t, t=t
                )[0],
                ModelMeanType.START_X: x_start_new,
                ModelMeanType.EPSILON: noise,
            }[self.model_mean_type]
            assert model_output_new.shape == target.shape == x_start_new.shape  # [bs, njoints, nfeats, nframes]

            terms["rot_mse"] = self.masked_l2(target, model_output_new, mask) # mean_flat(rot_mse)

            target_xyz, model_output_xyz = None, None

            if self.lambda_rcxyz > 0.:
                target_xyz = get_xyz(target)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes]
                model_output_xyz = get_xyz(model_output_new)  # [bs, nvertices, 3, nframes]
                terms["rcxyz_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d' and dataset.dataname in ['humanact12', 'uestc']:
                    target_xyz = get_xyz(target) if target_xyz is None else target_xyz
                    model_output_xyz = get_xyz(model_output_new) if model_output_xyz is None else model_output_xyz
                    target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                    model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                    terms["vel_xyz_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            ...
            if self.lambda_vel > 0.:
                target_vel = (target[..., 1:] - target[..., :-1])
                model_output_vel = (model_output_new[..., 1:] - model_output_new[..., :-1])
                terms["vel_mse"] = self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)
