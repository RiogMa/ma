...
class GaussianDiffusion:
    ...
    def training_losses(self, model, body_vip, body_not, name, t, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            body_vip_output = model(body_vip_t, self._scale_timesteps(t), **model_kwargs)
            ...
            target_not = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=body_not, x_t=body_not_t, t=t
                )[0],
                ModelMeanType.START_X: body_not,
                ModelMeanType.EPSILON: noise,
            }[self.model_mean_type]
            
            assert body_vip_output.shape == target_vip.shape == body_vip.shape  # [bs, njoints, nfeats, nframes]
            ...
            非核心部位损失
            '''
            terms["rot_mse_not"] = self.masked_l2(target_not, body_not_output, mask) # mean_flat(rot_mse) 
            
            if self.lambda_rcxyz > 0.:
                target_xyz = get_xyz(target_not)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes]
                model_output_xyz = get_xyz(body_not_output)  # [bs, nvertices, 3, nframes]
                terms["rcxyz_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            if self.lambda_vel > 0.:
                target_xyz = get_xyz(target_not) 
                model_output_xyz = get_xyz(body_not_output)
                target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                terms["vel_mse_not"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])

            terms["loss_not"] = terms["rot_mse_not"]+\
                            (self.lambda_vel * terms.get('vel_mse_not', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_mse_not', 0.))
