...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start_new, x_start_old, t, model_kwargs=None, noise_new=None, noise_old=None, dataset=None):
        """
        ...
        if model_kwargs is None:
            model_kwargs = {}
        # 分别加噪
        if noise_new is None:
            noise_new = th.randn_like(x_start_new)
        if noise_old is None:
            noise_old = th.randn_like(x_start_old)
        x_t_new = self.q_sample(x_start_new, t, noise=noise_new)
        x_t_old = self.q_sample(x_start_old, t, noise=noise_old)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            terms_new["loss"] = self._vb_terms_bpd(
                model=model,
                x_start=x_start_new,
                x_t=x_t_new,
                t=t,
            ...
            if self.loss_type == LossType.RESCALED_KL:
                terms_new["loss"] *= self.num_timesteps
            ...
            if self.model_var_type in [
                ...
                B, C = x_t_new.shape[:2]
                assert model_output.shape == (B, C * 2, *x_t_new.shape[2:])
                model_output, model_var_values = th.split(model_output, C, dim=1)
                # Learn the variance using the variational bound, but don't let
                # it affect our mean prediction.
                frozen_out = th.cat([model_output.detach(), model_var_values], dim=1)
                terms_new["vb"] = self._vb_terms_bpd(
                ...
                if self.loss_type == LossType.RESCALED_MSE:
                    ...
                    terms_new["vb"] *= self.num_timesteps / 1000.0

            target = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=x_start_new, x_t=x_t_new, t=t
                )[0],
                ModelMeanType.START_X: x_start_new,
                ModelMeanType.EPSILON: noise_new,
            }[self.model_mean_type]
            assert model_output.shape == target.shape == x_start_new.shape  # [bs, njoints, nfeats, nframes]

            terms_new["rot_mse"] = self.masked_l2(target, model_output, mask) # mean_flat(rot_mse)
            ...
            if self.lambda_rcxyz > 0.:
                ...
                terms_new["rcxyz_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)
            ...
            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d' and dataset.dataname in ['humanact12', 'uestc']:
                    ...
                    terms_new["vel_xyz_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            ...
            if self.lambda_fc > 0.:
                ...
                if self.data_rep == 'rot6d' and dataset.dataname in ['humanact12', 'uestc']:
                    ...
                    pred_vel[~fc_mask] = 0
                    terms_new["fc"] = self.masked_l2(pred_vel,
                                                 torch.zeros(pred_vel.shape, device=pred_vel.device),
                                                 mask[:, :, :, 1:])
            if self.lambda_vel > 0.:
                target_vel = (target[..., 1:] - target[..., :-1])
                model_output_vel = (model_output[..., 1:] - model_output[..., :-1])
                terms_new["vel_mse"] = self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)

            terms_new["loss"] = terms_new["rot_mse"] + terms_new.get('vb', 0.) +\
                            (self.lambda_vel * terms_new.get('vel_mse', 0.)) +\
                            (self.lambda_rcxyz * terms_new.get('rcxyz_mse', 0.)) + \
                            (self.lambda_fc * terms_new.get('fc', 0.))
        else:
        ...
        '''
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            terms_old["loss"] = self._vb_terms_bpd(
                model=model,
                x_start=x_start_old,
                x_t=x_t_old,
                t=t,
            ...
            if self.loss_type == LossType.RESCALED_KL:
                terms_old["loss"] *= self.num_timesteps
        elif self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            model_output = model(x_t_old, self._scale_timesteps(t), **model_kwargs)
            ...
            if self.model_var_type in [
                ...
                B, C = x_t_old.shape[:2]
                assert model_output.shape == (B, C * 2, *x_t_old.shape[2:])
                model_output, model_var_values = th.split(model_output, C, dim=1)
                # Learn the variance using the variational bound, but don't let
                # it affect our mean prediction.
                frozen_out = th.cat([model_output.detach(), model_var_values], dim=1)
                terms_old["vb"] = self._vb_terms_bpd(
                    model=lambda *args, r=frozen_out: r,
                    x_start=x_start_old,
                    x_t=x_t_old,
                    t=t,
                ...
                if self.loss_type == LossType.RESCALED_MSE:
                    # Divide by 1000 for equivalence with initial implementation.
                    # Without a factor of 1/1000, the VB term hurts the MSE term.
                    terms_old["vb"] *= self.num_timesteps / 1000.0

            target = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=x_start_old, x_t=x_t_old, t=t
                )[0],
                ModelMeanType.START_X: x_start_old,
                ModelMeanType.EPSILON: noise_old,
            }[self.model_mean_type]
            assert model_output.shape == target.shape == x_start_old.shape  # [bs, njoints, nfeats, nframes]

            terms_old["rot_mse"] = self.masked_l2(target, model_output, mask) # mean_flat(rot_mse)
            ...
            if self.lambda_rcxyz > 0.:
                target_xyz = get_xyz(target)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes]
                model_output_xyz = get_xyz(model_output)  # [bs, nvertices, 3, nframes]
                terms_old["rcxyz_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)
            ...
            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d' and dataset.dataname in ['humanact12', 'uestc']:
                    ...
                    target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                    model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                    terms_old["vel_xyz_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            ...
            if self.lambda_fc > 0.:
                ...
                if self.data_rep == 'rot6d' and dataset.dataname in ['humanact12', 'uestc']:
                    ...
                    pred_vel = pred_joint_xyz[:, :, :, 1:] - pred_joint_xyz[:, :, :, :-1]
                    pred_vel[~fc_mask] = 0
                    terms_old["fc"] = self.masked_l2(pred_vel,
                                                 torch.zeros(pred_vel.shape, device=pred_vel.device),
                                                 mask[:, :, :, 1:])
            if self.lambda_vel > 0.:
                target_vel = (target[..., 1:] - target[..., :-1])
                model_output_vel = (model_output[..., 1:] - model_output[..., :-1])
                terms_old["vel_mse"] = self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)

            terms_old["loss"] = terms_old["rot_mse"] + terms_old.get('vb', 0.) +\
                            (self.lambda_vel * terms_old.get('vel_mse', 0.)) +\
                            (self.lambda_rcxyz * terms_old.get('rcxyz_mse', 0.)) + \
                            (self.lambda_fc * terms_old.get('fc', 0.))
            ...
            raise NotImplementedError(self.loss_type)
        # print("new",terms_new["loss"])
        # print("old:",terms_old["loss"])
        terms = {}
        terms["loss"] = terms_new["loss"] + 0.2*terms_old["loss"]
