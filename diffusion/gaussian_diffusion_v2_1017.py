...
class GaussianDiffusion:
    ...
    def training_losses(self, model, body_lul_0, body_rul_0, body_lll_0, body_rll_0, body_torso_0, body_parts, t, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        # if noise is None:
        #     noise = th.randn_like(x_start)
        noise_lul = th.randn_like(body_lul_0)
        noise_rul = th.randn_like(body_rul_0)
        noise_lll = th.randn_like(body_lll_0)
        noise_rll = th.randn_like(body_rll_0)
        noise_torso = th.randn_like(body_torso_0)
        
        body_lul_t = self.q_sample(body_lul_0, t, noise=noise_lul)
        body_rul_t = self.q_sample(body_rul_0, t, noise=noise_rul)
        body_lll_t = self.q_sample(body_lll_0, t, noise=noise_lll)
        body_rll_t = self.q_sample(body_rll_0, t, noise=noise_rll)
        body_torso_t = self.q_sample(body_torso_0, t, noise=noise_torso)
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            body_lul_output = model(body_lul_t, self._scale_timesteps(t), **model_kwargs)
            body_rul_output = model(body_rul_t, self._scale_timesteps(t), **model_kwargs)
            body_lll_output = model(body_lll_t, self._scale_timesteps(t), **model_kwargs)
            body_rll_output = model(body_rll_t, self._scale_timesteps(t), **model_kwargs)
            body_torso_output = model(body_torso_t, self._scale_timesteps(t), **model_kwargs)
            ...
            target_lul = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=body_lul_0, x_t=body_lul_t, t=t
                )[0],
                ModelMeanType.START_X: body_lul_0,
                ModelMeanType.EPSILON: noise,
            }[self.model_mean_type]
            assert body_lul_output.shape == target_lul.shape == body_lul_0.shape  # [bs, njoints, nfeats, nframes]
            target_rul = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=body_rul_0, x_t=body_rul_t, t=t
                )[0],
                ModelMeanType.START_X: body_rul_0,
                ModelMeanType.EPSILON: noise,
            }[self.model_mean_type]
            assert body_rul_output.shape == target_rul.shape == body_rul_0.shape  # [bs, njoints, nfeats, nframes]
            target_lll = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=body_lll_0, x_t=body_lll_t, t=t
                )[0],
                ModelMeanType.START_X: body_lll_0,
                ModelMeanType.EPSILON: noise,
            }[self.model_mean_type]
            assert body_lll_output.shape == target_lll.shape == body_lll_0.shape  # [bs, njoints, nfeats, nframes]
            target_rll = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=body_rll_0, x_t=body_rll_t, t=t
                )[0],
                ModelMeanType.START_X: body_rll_0,
                ModelMeanType.EPSILON: noise,
            }[self.model_mean_type]
            assert body_rll_output.shape == target_rll.shape == body_rll_0.shape  # [bs, njoints, nfeats, nframes]
            target_torso = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=body_torso_0, x_t=body_torso_t, t=t
                )[0],
                ModelMeanType.START_X: body_torso_0,
                ModelMeanType.EPSILON: noise,
            }[self.model_mean_type]
            assert body_torso_output.shape == target_torso.shape == body_torso_0.shape  # [bs, njoints, nfeats, nframes]
            ...
            lul 损失
            '''
            terms["rot_lul_mse"] = self.masked_l2(target_lul, body_lul_output, mask) # mean_flat(rot_mse)
            if self.lambda_rcxyz > 0.:
                target_xyz = get_xyz(target_lul)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes]
                model_output_xyz = get_xyz(body_lul_output)  # [bs, nvertices, 3, nframes]
                terms["rcxyz_lul_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d':
                    target_xyz = get_xyz(target_lul)
                    model_output_xyz = get_xyz(body_lul_output)
                    target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                    model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                    terms["vel_lul_xyz_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])

            if self.lambda_vel > 0.:
                target_vel = (target_lul[..., 1:] - target_lul[..., :-1])
                model_output_vel = (body_lul_output[..., 1:] - body_lul_output[..., :-1])
                terms["vel_lul_mse"] = self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)
            terms["lul_loss"] = terms["rot_lul_mse"] +\
                            (self.lambda_vel * terms.get('vel_lul_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_lul_mse', 0.)) 
            '''
            rul 损失
            '''    
            terms["rot_rul_mse"] = self.masked_l2(target_rul, body_rul_output, mask) # mean_flat(rot_mse)
            if self.lambda_rcxyz > 0.:
                target_xyz = get_xyz(target_rul)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes]
                model_output_xyz = get_xyz(body_rul_output)  # [bs, nvertices, 3, nframes]
                terms["rcxyz_rul_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d':
                    target_xyz = get_xyz(target_rul) if target_xyz is None else target_xyz
                    model_output_xyz = get_xyz(body_rul_output) if model_output_xyz is None else model_output_xyz
                    target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                    model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                    terms["vel_rul_xyz_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])

            if self.lambda_vel > 0.:
                target_vel = (target_rul[..., 1:] - target_rul[..., :-1])
                model_output_vel = (body_rul_output[..., 1:] - body_rul_output[..., :-1])
                terms["vel_rul_mse"] = self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)
            terms["rul_loss"] = terms["rot_rul_mse"] +\
                            (self.lambda_vel * terms.get('vel_rul_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_rul_mse', 0.)) 
                
            '''
            lll 损失
            '''    
            terms["rot_lll_mse"] = self.masked_l2(target_lll, body_lll_output, mask) # mean_flat(rot_mse)
            if self.lambda_rcxyz > 0.:
                target_xyz = get_xyz(target_lll)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes]
                model_output_xyz = get_xyz(body_lll_output)  # [bs, nvertices, 3, nframes]
                terms["rcxyz_lll_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d':
                    target_xyz = get_xyz(target_lll) 
                    model_output_xyz = get_xyz(body_lll_output)
                    target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                    model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                    terms["vel_lll_xyz_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            ...
            if self.lambda_fc > 0.:
                ...
                if self.data_rep == 'rot6d':
                    ...
                    pred_vel[~fc_mask] = 0
                    terms["fc_lll"] = self.masked_l2(pred_vel,
                                                 torch.zeros(pred_vel.shape, device=pred_vel.device),
                                                 mask[:, :, :, 1:])
            if self.lambda_vel > 0.:
                target_vel = (target_lll[..., 1:] - target_lll[..., :-1])
                model_output_vel = (body_lll_output[..., 1:] - body_lll_output[..., :-1])
                terms["vel_lll_mse"] = self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)
            
            terms["lll_loss"] = terms["rot_lll_mse"]+\
                            (self.lambda_vel * terms.get('vel_lll_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_lll_mse', 0.)) + \
                            (self.lambda_fc * terms.get('fc_lll', 0.))
            '''
            rll 损失
            '''   
            terms["rot_rll_mse"] = self.masked_l2(target_rll, body_rll_output, mask) # mean_flat(rot_mse)
            if self.lambda_rcxyz > 0.:
                target_xyz = get_xyz(target_rll)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes]
                model_output_xyz = get_xyz(body_rll_output)  # [bs, nvertices, 3, nframes]
                terms["rcxyz_rll_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d':
                    target_xyz = get_xyz(target_rll)
                    model_output_xyz = get_xyz(body_rll_output)
                    target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                    model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                    terms["vel_rll_xyz_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])

            if self.lambda_fc > 0.:
                torch.autograd.set_detect_anomaly(True)
                if self.data_rep == 'rot6d':
                    target_xyz = get_xyz(target_rll) if target_xyz is None else target_xyz
                    model_output_xyz = get_xyz(body_rll_output) if model_output_xyz is None else model_output_xyz
                    # 'L_Ankle',  # 7, 'R_Ankle',  # 8 , 'L_Foot',  # 10, 'R_Foot',  # 11
                    l_ankle_idx, r_ankle_idx, l_foot_idx, r_foot_idx = 7, 8, 10, 11
                    relevant_joints = [l_ankle_idx, l_foot_idx, r_ankle_idx, r_foot_idx]
                    gt_joint_xyz = target_xyz[:, relevant_joints, :, :]  # [BatchSize, 4, 3, Frames]
                    ...
                    pred_vel = pred_joint_xyz[:, :, :, 1:] - pred_joint_xyz[:, :, :, :-1]
                    pred_vel[~fc_mask] = 0
                    terms["fc_rll"] = self.masked_l2(pred_vel,
                                                 torch.zeros(pred_vel.shape, device=pred_vel.device),
                                                 mask[:, :, :, 1:])
            if self.lambda_vel > 0.:
                target_vel = (target_rll[..., 1:] - target_rll[..., :-1])
                model_output_vel = (body_rll_output[..., 1:] - body_rll_output[..., :-1])
                terms["vel_rll_mse"] = self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)
            terms["rll_loss"] = terms["rot_rll_mse"]+\
                            (self.lambda_vel * terms.get('vel_rll_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_rll_mse', 0.)) + \
                            (self.lambda_fc * terms.get('fc_rll', 0.))
            '''
            torso 损失
            '''    
            terms["rot_torso_mse"] = self.masked_l2(target_torso, body_torso_output, mask) # mean_flat(rot_mse)
            if self.lambda_rcxyz > 0.:
                target_xyz = get_xyz(target_torso)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes]
                model_output_xyz = get_xyz(body_torso_output)  # [bs, nvertices, 3, nframes]
                terms["rcxyz_torso_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d':
                    target_xyz = get_xyz(target_torso) if target_xyz is None else target_xyz
                    model_output_xyz = get_xyz(body_torso_output) if model_output_xyz is None else model_output_xyz
                    target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                    model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                    terms["vel_torso_xyz_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])

            if self.lambda_vel > 0.:
                target_vel = (target_torso[..., 1:] - target_torso[..., :-1])
                model_output_vel = (body_torso_output[..., 1:] - body_torso_output[..., :-1])
                terms["vel_torso_mse"] = self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)                                                                

            terms["torso_loss"] = terms["rot_torso_mse"] +\
                            (self.lambda_vel * terms.get('vel_torso_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_torso_mse', 0.)) 
                            
            '''
            汇总损失函数
            '''
            bs = body_lll_0.shape[0]
            # print(bs)
            lul_loss = [0.0] * bs
            rul_loss = [0.0] * bs
            lll_loss = [0.0] * bs
            ...
            # 遍历每个batch_size
            for i in range(bs):
                # 判断body_parts_list中是否包含特定部位，若包含则设置相应的loss值
                if 'left upper limb' in body_parts[i]:
                    lul_loss[i] = lamada_body
                else:
                    lul_loss[i] = lamada_general

                if 'right upper limb' in body_parts[i]:
                    rul_loss[i] = lamada_body
                else:
                    rul_loss[i] = lamada_general

                if 'left lower limb' in body_parts[i]:
                    lll_loss[i] = lamada_body
                else:
                    lll_loss[i] = lamada_general

                if 'right lower limb' in body_parts[i]:
                    rll_loss[i] = lamada_body
                else:
                    rll_loss[i] = lamada_general

                if 'torso' in body_parts[i]:
                    torso_loss[i] = lamada_body
                else:
                    torso_loss[i] = lamada_general
            
            rul_loss = torch.tensor(rul_loss)
            lul_loss = torch.tensor(lul_loss)
            lll_loss = torch.tensor(lll_loss)
            rll_loss = torch.tensor(rll_loss)
            torso_loss = torch.tensor(torso_loss)
            # print(torch.cuda.is_available())
            rul_loss = rul_loss.to("cuda:0")
            lul_loss = lul_loss.to("cuda:0")
            lll_loss = lll_loss.to("cuda:0")
            rll_loss = rll_loss.to("cuda:0")
            torso_loss = torso_loss.to("cuda:0")

            terms["loss"] = lul_loss * terms.get("lul_loss") +\
                            rul_loss * terms.get("rul_loss")+\
                            lll_loss * terms.get("lll_loss")+\
                            rll_loss * terms.get("rll_loss")+\
                            torso_loss * terms.get("torso_loss")
