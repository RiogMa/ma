...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.lambda_general = lambda_general
    ...
    def training_losses(self, model, body_lul_0, body_rul_0, body_lll_0, body_rll_0, body_torso_0, body_parts, t, model_kwargs=None, noise=None, dataset=None):
        ...
        def get_left_foot(target):
            return target[:, 22:28, :, :]
        
        def get_right_foot(target):
            return target[:, 31:37, :, :]
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.lambda_vel > 0.:
                ...
                terms["vel_lul_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            ...
            if self.lambda_vel > 0.:
                ...
                target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                terms["vel_rul_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            ...
            if self.lambda_vel > 0.:
                if self.data_rep == 'rot6d':
                    ...
                    target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                    model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                    terms["vel_lll_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            ...
            if self.lambda_fc > 0.:
                ...
                if self.data_rep == 'rot6d':
                    ...
                    lf = get_left_foot(target_xyz)
                    rf = get_right_foot(target_xyz)
                    gt_joint_xyz = torch.cat((lf, rf), dim=1)  # [BatchSize, 4, 3, Frames]
                    ...
                    plf = get_left_foot(model_output_xyz)
                    prf = get_right_foot(model_output_xyz)
                    pred_joint_xyz = torch.cat((plf, prf), dim=1)  # [BatchSize, 4, 3, Frames]
                    pred_vel = torch.linalg.norm(pred_joint_xyz[:, :, :, 1:] - pred_joint_xyz[:, :, :, :-1], axis=2)  # [BatchSize, 4, Frames]
                    # pred_vel[~fc_mask] = 0
            ...
            # print("body", self.lambda_body)
            # print("general", self.lambda_general)
            # print("fc", self.lambda_fc)
            # 遍历每个batch_size
            for i in range(bs):
                # 判断body_parts_list中是否包含特定部位，若包含则设置相应的loss值
                if 'left upper limb' in body_parts[i]:
                    lul_loss[i] = self.lambda_body
                else:
                    lul_loss[i] = self.lambda_general

                if 'right upper limb' in body_parts[i]:
                    rul_loss[i] = self.lambda_body
                else:
                    rul_loss[i] = self.lambda_general

                if 'left lower limb' in body_parts[i]:
                    lll_loss[i] = self.lambda_body
                else:
                    lll_loss[i] = self.lambda_general

                if 'right lower limb' in body_parts[i]:
                    rll_loss[i] = self.lambda_body
                else:
                    rll_loss[i] = self.lambda_general

                if 'torso' in body_parts[i]:
                    torso_loss[i] = self.lambda_body
                else:
                    torso_loss[i] = self.lambda_general
